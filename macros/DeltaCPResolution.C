...
void FillChiSquaredProfileTree(const NeutrinoEventVector &nuEventVector, TH1D *& nueEnergyDistribution_observed, const int deltaCPIndex, 
    const int sin2Theta23Index, TTree *& chiSquaredProfileTree);
...
const int N_DELTA_CP_VALUES = 10;
const double DELTA_CP_MIN = 0.0; //- 1.0 * M_PI;
...
const double DEF_SIN2THETA23 = 0.5;
...
void PlotResolution(const NeutrinoEventVector &nuEventVector)
{
    ...
    std::cout << "deltaCP / DELTA_CP_STEP_SIZE: " << (deltaCP - DELTA_CP_MIN) / DELTA_CP_STEP_SIZE << std::endl;
    ...
    const int sin2Theta23Index, TTree *& chiSquaredProfileTree)
{
    for (int deltaCPOffset = -1; deltaCPOffset <= 1; ++deltaCPOffset)
    {
        ...
        for (int sin2Theta23Offset = -1; sin2Theta23Offset <= 1; ++sin2Theta23Offset)
        {
            ...
            // get chi-squared
            FillChiSquaredProfileTree(nuEventVector, nueEnergyDistribution_observed, neighbourDeltaCPIndex, neighbourSin2Theta23Index, chiSquaredProfileTree);
